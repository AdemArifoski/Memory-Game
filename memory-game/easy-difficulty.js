...
function createGame(containerId, className, resetButton, timerId, modalContainerId,gameOverId, totalPairs, levelNumber){
  ...
  function resetGame(){
    ...
    // Call displayNumbers again to reattach clean event listeners
    displayNumbers();
  }
  ...
  const totalLivesKey = "totalLives";
...
createGame("numberContainer1", ".number1","resetButton1", "timer1", "modalContainer1", "gameOver1", 10, 1);
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  for (let level = 1; level <= 5; level++) {
    const highScoreKey = `highScoreLvl${level}`;
    const bestMovesKey = `bestMovesLvl${level}`;
...
function changeBackToLevelOne() {
  ...
  // Reset event listeners for next level button 1 to 4
  for (let i = 1; i <= 4; i++) {
...
function changeLevel(level) {
  ...
  createGame(numberContainerId, numberClass, resetButton, timerId, modalContainerId, gameOverId, numberCount, levelNumber);
